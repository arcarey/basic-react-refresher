...
describe('StoryDetail', () => {
  ...
  it('renders the story content', async () => {
    jest.spyOn(container, 'fetchStories').mockImplementation(() => 
      Promise.resolve({
        json: () => Promise.resolve(mockEndpointResponse)
      }))
    ...
    expect(container.querySelector('h2').textContent.toBe(mockEndpointResponse.title))
  })
